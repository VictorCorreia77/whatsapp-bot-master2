...
const start = (aruga = new Client()) => {
	...
	// kondisi ketika batas member group belum tercapai, ubah di file settings/setting.json
        ...
        await aruga.simulateTyping(chat.id, true).then(async () => {
          await aruga.sendText(chat.id, `Olá, sou o Bot de Victor, para ver os comandos digite: ${prefix}menu`)
        })
	    }
	}
    })
    ...
    aruga.onGlobalParicipantsChanged(async (event) => {
        ...
        if (event.action === 'add' && event.who !== host && isWelcome) {
            ...
            await aruga.sendTextWithMentions(event.chat, `Olá, bem-vimdo @${event.who.replace('@c.us', '')} \n\nSeja feliz conosco`)
        }
        // kondisi ketika seseorang dikick/keluar dari group
        if (event.action === 'remove' && event.who !== host) {
			await aruga.sendFileFromUrl(event.chat, profile, 'profile.jpg', '')
            await aruga.sendTextWithMentions(event.chat, `Adeus @${event.who.replace('@c.us', '')}, Sentiremos sua falta✨`)
        }
    })
