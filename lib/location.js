    ...
    return new Promise((resolve, reject) => {
        ...
        fetchJson('https://api.pedulilindungi.id/zone/v1', options)
            .then(json => {
                ...
                case 'red':
                    result.status = 'Você está na Zona Vermelha da propagação de COVID-19.'
                    result.optional = 'A Zona Vermelha é uma área onde já existem casos positivos de COVID-19.'
                    break
                case 'yellow':
                    result.status = 'Você está na Zona Amarela da propagação de COVID-19.'
                    result.optional = 'A Zona Amarela é uma área onde há casos de ODP ou PDP COVID-19.'
                    break
                case 'green':
                    result.status = 'Você está na Zona Verde da propagação de COVID-19.'
                    result.optional = 'A Zona Verde é uma área onde não há casos de PDP ou COVID-19 Positivo.'
                    break
                }

                if (!json.success && json.message == 'Você está em uma zona segura.') {
                    result.kode = 'green'
                    result.status = 'Você está na Zona Verde da propagação do COVID-19.'
                    result.optional = 'A Zona Verde é uma área onde não há casos de PDP ou COVID-19 Positivo.'
                }
                resolve(result)
            })
